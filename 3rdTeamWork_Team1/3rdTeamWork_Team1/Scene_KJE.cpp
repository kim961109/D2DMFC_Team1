...
void CScene_KJE::Initialize(void)
{
	...
	CObjMgr::Get_Instance()->Add_Object(OBJ_ITEM, CAbstractFactory<CApple>::Create(300.f, 300.f));
}
...
void CScene_KJE::Update(void)
{
	...
	{
		CObjMgr::Get_Instance()->Add_Object(OBJ_UI, CAbstractFactory<CGameOver>::Create(0, 0));
	}
	else
	{
...
void CScene_KJE::Render(HDC hDC)
{
	...
	TextOut(hDC, 650, 65, szApple, lstrlen(szApple));
	//타이머
	TCHAR	szBuff[32] = L"";
	swprintf_s(szBuff, L"Timer %d : %d", m_iSecond, m_iMilliSecond);
	TextOut(hDC, 70, 65, szBuff, lstrlen(szBuff));
